...
-- a)
-- 1. select * from Bok where forlag_id = 2 projeksjon
-- 2. select distinct bok_id, tittel from bok seleksjon

--b)
-- select * from Forlag, Bok
-- antall tupler i forlag * antall tupler i bok = antall tupler i resultat
-- antall attributter i resultatrelasjonen er lik summen av antall attributter i A og antall attributter i B

--c)
-- 1. select * from Forlag join Bok on forlag.forlag_id = bok.forlag_id
-- 2. select * from Forlag natural join Bok
-- resultatet forteller oss hvilke bøker som tilhører hvilke forlag_id

--d)
-- unionkompatibell: samme antallet attributter, og at attributtene er definert på samme domene
-- union, snitt, differanse
-- select forlag_id, forlag_navn from forlag union select bok_id, tittel from bok
-- select * from bok inner join forlag where forlag.forlag_id = bok.forlag_id
-- select distinct * from bok join forlag on (forlag.forlag_id = bok.forlag_id) snitt
...
--b)
-- SELECT forlag_id FROM forlag LEFT join bok using (forlag_id) where bok.forlag_id is null
-- ytterforening
...
--d)
-- select forlag_navn, adresse from forlag, bok where bok.tittel = 'Generation X' and forlag.forlag_id = bok.forlag_id
-- produkt

--e)
-- select tittel from bok, bok_forfatter, forfatter where forfatter.etternavn = 'Hamsun' and bok.bok_id = bok_forfatter.bok_id and bok_forfatter.forfatter_id = forfatter.forfatter_id
-- select tittel from bok natural join bok_forfatter natural join forfatter where forfatter.etternavn = "Hamsun"
-- produkt

--f)
-- select tittel, utgitt_aar, forlag_navn, adresse, telefon from bok, forlag, bok_forfatter where forlag.forlag_id=bok.forlag_id and bok.bok_id=bok_forfatter.bok_id
